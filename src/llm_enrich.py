...
def calculate_metrics(df, N=30):
    ...
    # Último preço por moeda
    last_date = df['date'].max()
    current = df[df['date'] == last_date][['target_currency', 'rate']].set_index('target_currency')

    # Preço do mesmo dia do mês passado (ou média do mês passado)
    ...
    # Top movers (maior mudança absoluta)
...
def call_llm(prompt, run_id):
    ...
    if not text:
        # Se a resposta estiver vazia, tenta reduzir prompt
        prompt_reduced = prompt[:2000]  # exemplo de truncamento
        return call_llm(prompt_reduced, run_id)
    
    return text

def enrich_with_llm(df, run_id):
